# Dashboard Home - Panel Principal

## Información General

| Campo | Valor |
|-------|-------|
| **Página** | dashboard-home |
| **Categoría** | dashboard |
| **URL** | /(dashboard)/dashboard-home |

## Descripción

Panel principal del usuario con resumen de actividad, propiedades recientes, mensajes y estadísticas.

## Recursos

- **Video**: `videos/` - Grabación WebM
- **Imágenes**: `images/` - Capturas de pantalla
- **Datos**: `docs/` - JSON y documentación técnica

## Ruta de Código

`src/app/[locale]//(dashboard)/dashboard-home/page.tsx`

## Reconstrucción

1. Copiar estructura de `page.tsx`
2. Importar componentes de `@/components/`
3. Configurar datos mock o API
4. Verificar responsividad móvil/desktop
5. Comparar con video e imágenes de referencia
